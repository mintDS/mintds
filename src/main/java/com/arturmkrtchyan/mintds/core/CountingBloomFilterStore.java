...
class CountingBloomFilterStore extends AbstractKeyValueStore<CountingBloomFilter> {
    ...
    @Override
    public Response count(final CountingBloomFilter filter, final Request request) {
        return new NumericResponse<>(filter.count(request.getValue().get()));
    }
    ...
    protected CountingBloomFilter newElement() {
        return new CountingBloomFilter(1024, 1d);
    }
